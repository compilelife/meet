...
function generateDots(paramDotsOptions, paramInt1, paramInt2) {
    ...
    while (paramInt2 <= i) {
      ...
      dotOption.rotation = getRotation();
...
export default {
    ...
    generate: async function(g, ctx){
        g.rect(ctx.width, ctx.height).fill(
            ctx.colors.shades('primary').light.hex
        )
        ...
        dots.forEach(dot=>{
            let show = true
            let color = ctx.colors.shades('primary').main.hex
            if (mask) {
                show = mask.isOpacity(dot.x, dot.y)
                if (mask.colorful) {
                    color = mask.colorAt(dot.x, dot.y).object()
                }
            }
            if (show) {
